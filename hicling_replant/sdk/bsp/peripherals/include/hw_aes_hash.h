...
/**
 * \brief Enable crypto engine clock
 *
 * This function enables the crypto engine clock.
 */
...
/**
 * \brief Disable crypto engine clock
 *
 * This function disables the crypto engine clock.
 */
 ...
 *
 * This function sets up and starts a crypto engine operation. All the
 ...
 *
 * This function sets up a crypto engine operation. All the configuration details
 ...
 *
 * This function disables the crypto engine and its interrupt.
 *
 * \param [in] waitTillInactive When true the crypto engine is disabled after any pending operation
...
/**
 * \brief Store initialization vector in crypto engine memory
 *
...
/**
 * \brief Store counter initialization in crypto engine memory
 *
...
/**
 * \brief Store AES keys in crypto engine memory
 *
 * This function stores the keys used for AES encryption/decryption in crypto engine memory.
 ...
 * need storing it to the crypto memory again.
 *
 * \param[in] key_size The size of the key used in the encryption/decryption
 ...
 * \param[in] dst The physical address (RAM only) of the buffer where the output of the operation will
 *                be stored. The dst address must be NULL when configuring the DMA while the engine is
 *                waiting for more input data.
 * \param[in] len The length of the input data
 */
void hw_aes_hash_cfg_dma(const uint8 *src, uint8 *dst, unsigned int len);

/**
 * \brief Start crypto engine operation
 *
 * This function starts a crypto operation. The operation depends on the configuration that
...
/**
 * \brief Enable crypto engine interrupt
 *
 * This function enables the crypto engine interrupt and sets a callback function
...
/**
 * \brief Disable crypto engine interrupt
 *
 * This function disables the crypto engine interrupt.
 */
 ...
 *
 * This function configures the crypto engine to write back to memory all the
 * resulting data.
 *
 * \note
 ...
 *
 * This function configures the crypto engine to write back to memory only the
