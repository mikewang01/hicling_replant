...
static void hw_spi_transfer_read(HW_SPI_ID id)
{
        ...
        /* we want dummy to be in RAM, to avoid extra QSPI flash accesses; so, volatile! */
        ...
        while (spid->tx_ix < spid->rx_len) {
                ...
                hw_spi_write_word(id, (uint8_t *) &dummy, wordsize);
                spid->tx_ix += wordsize;
        }
}
                                                             ...
                                                             hw_spi_tx_callback cb, void *user_data)
{
        ...
        if (cb == NULL) {
                ...
                if (!hw_spi_is_slave(id)) {
                        spid->tx_dma.src_address = (uint32_t)&dummy;
                        spid->tx_dma.length = len / wordsize;
                        /*
