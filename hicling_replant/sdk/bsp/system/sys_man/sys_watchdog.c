...
PRIVILEGED_DATA static TimerHandle_t tasks_notify_timer;
#endif
...
#if dg_configWDOG_NOTIFY_TRIGGER_TMO
static void watchdog_auto_notify_cb(TimerHandle_t timer)
{
        int i;
...
#if dg_configWDOG_NOTIFY_TRIGGER_TMO
        tasks_notify_timer = xTimerCreate("wdog", OS_MS_2_TICKS(dg_configWDOG_NOTIFY_TRIGGER_TMO),
...
int8_t sys_watchdog_register(bool notify_trigger)
{
        ...
        if (notify_trigger) {
                ...
                if (!tasks_notify_mask) {
                        xTimerStart(tasks_notify_timer, 0);
                }
...
void sys_watchdog_unregister(int8_t id)
{
        ...
        if (!tasks_notify_mask) {
                xTimerStop(tasks_notify_timer, 0);
        }
#endif
