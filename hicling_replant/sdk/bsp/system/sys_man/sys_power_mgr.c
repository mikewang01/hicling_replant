...
PRIVILEGED_DATA uint32_t pm_mac_wakeup_time[PM_MAX_ID - PM_BASE_ID]; // 0: ID #0
                                                                     // 1: ID #1, etc.
...
void pm_system_init(periph_init_cb peripherals_initialization)
{
        ...
        if ((dg_configCODE_LOCATION == NON_VOLATILE_IS_FLASH) || (dg_configPOWER_FLASH == 1)) {
                ...
                /* When in Development mode, copy QFIS code since the Boot code does not do
                 * it for serial boot (OTP is left active from the Boot code in manual
                 * read).
                 */
                if ((dg_configWRITE_QFIS_UCODE)
...
void pm_resource_sleeps_until(pm_id_t id, uint32_t time_in_LP_cycles)
{
        ...
        lp_event_time = rtc_time + time_in_LP_cycles - dg_configWAKEUP_XTAL16_TIME;
        if (lp_event_time == 0) {
...
/**
 * \brief Return the wakeup time for MAC with id
 *
...
uint32_t pm_get_mac_wakeup_time(pm_id_t id)
{
        ...
        return pm_mac_wakeup_time[id - PM_BASE_ID] + dg_configWAKEUP_XTAL16_TIME;
}
...
__RETAINED_CODE static uint32_t pm_advance_time_compute(uint32_t prescaled_time, uint32_t *trigger)
{
        ...
        tick_offset = elapsed_time % TICK_PERIOD;       // Offset in current tick (in prescaled LP cycles).
...
__RETAINED_CODE static void apply_wfi(bool allow_entering_sleep, uint32_t sleep_period)
{
        ...
        if (allow_entering_sleep) {
                ...
                /*
                 * Disable debugger pins and reconfigure (any) floating GPIOs to eliminate leakage.
                 */
                dg_codeCUT_DEBUGGER_PINS_LEAKAGE;
        ...
        /*
         * Disable the Watchdog if the debugger is on since the System PD will not be turned
         * off, causing the Watchdog to hit.
         */
        if ((dg_configUSE_WDOG == 1) && hw_cpm_is_debugger_attached()) {
                hw_watchdog_freeze();
        }
        ...
        /*
         * Re-enable the Watchdog in case it was turned off due to the debugger being attached.
         */
        if ((dg_configUSE_WDOG == 1) && hw_cpm_is_debugger_attached()) {
        ...
        if (pm_system_sleeping == sys_powered_down) {
                ...
                if ((dg_configFORCE_DEEP_SLEEP == 1)
                                && (pm_current_sleep_mode >= pm_mode_deep_sleep_no_mirror)
                                && (sleep_period == 0)) {
                ...
                if ((pm_current_sleep_mode >= pm_mode_deep_sleep_no_mirror)
                        ...
                        if (dg_configPOWER_CONFIG == POWER_CONFIGURATION_1) {
                                if (dg_configUSE_LP_CLK != LP_CLK_RCX) {
                                        ASSERT_WARNING(0);      // Not yet supported!
                                }
                                else {
                                        hw_cpm_enable_rcx();
                                        hw_cpm_lp_set_rcx();
...
void pm_sleep_enter(uint32_t low_power_periods)
{
        ...
        if (!abort_sleep && (pm_current_sleep_mode != pm_mode_active)) {
                ...
                uint32_t wakeup_time = dg_configWAKEUP_XTAL16_TIME;
                ...
                if (allow_entering_sleep) {                     // Power-down is possible.
                        ...
                        if (pm_mac_wakeup_time[0] != 0) {
                                ...
                                if (pm_mac_wakeup_time[0] < rtc_time) {
                                        ...
                                        rtc_offset = pm_mac_wakeup_time[0] - rtc_time;
