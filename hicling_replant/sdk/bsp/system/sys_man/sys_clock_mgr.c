...
#define RCX_MIN_TICK_CYCLES             19
#define RCX_MAX_TICK_CYCLES             23
 ...
 *
 */
static void cm_adjust_otp_access_timings(void)
{
        uint32_t clk_freq;
...
void cm_check_xtal_startup(void)
{
        ...
        uint32_t t1, t2;
        uint32_t rtc_start = rtc_get_fromCPM(&t1, &t2);
#endif
...
void cm_clk_init_low_level(void)
{
        ...
        // Low power clock
        if (dg_configUSE_LP_CLK == LP_CLK_RCX) {
                // Use the RC32K to count the XTAL16M settling time since the RCX is not stable yet.
                hw_cpm_enable_rc32k();
                hw_cpm_lp_set_rc32k();
                ...
                /* Use the RC32K to count the XTAL16M settling time since the XTAL32K is not stable
                 * yet.
                 */
                hw_cpm_enable_rc32k();
                hw_cpm_lp_set_rc32k();
        ...
        if (dg_configUSE_LP_CLK == LP_CLK_RCX) {
                ...
                for (i = 0; i < RCX_REPEAT_CALIBRATION_PUP; i++) {
                        ...
                        res = (16000000 * RCX_CALIBRATION_CYCLES_PUP) / cal_value;
                        hz_value += res;
                }
                correction = RCX_REPEAT_CALIBRATION_PUP / 2;
                rcx_clock_hz = (hz_value + correction) / RCX_REPEAT_CALIBRATION_PUP;
                rcx_clock_period = (uint32_t)(rcx_period_dividend / rcx_clock_hz);
                ble_slot_duration_in_rcx = 625 * rcx_clock_hz;
        ...
        if ((dg_configPOWER_CONFIG == POWER_CONFIGURATION_1)
                        && (dg_configPOWER_FLASH == 0)
                        && (dg_configPOWER_EXT_1V8_PERIPHERALS == 0)) {
                hw_cpm_reset_recharge_period();                 // Set the recharge period to 0
        }
        else {
                if (dg_configUSE_LP_CLK != LP_CLK_RCX) {
...
void cm_calibrate_rcx_start(void)
{
        hw_cpm_start_calibration(CALIBRATE_RCX, RCX_CALIBRATION_CYCLES_WUP);
...
static void rcx_calibration_task( void *pvParameters )
{
        ...
        while(1) {
                ...
                if (ulNotifiedValue & RCX_DO_CALIBRATION) {
                        cal_value = hw_cpm_get_calibration_data();
                        rcx_clock_hz = ((16000000 * RCX_CALIBRATION_CYCLES_WUP) + (cal_value >> 1)) / cal_value;
                        rcx_tick_rate_hz = get_optimum_tick_rate(rcx_clock_hz, &dummy);
                        rcx_clock_period = (uint32_t)(rcx_period_dividend / rcx_clock_hz);
                        ble_slot_duration_in_rcx = 625 * (uint32_t)rcx_clock_hz;
