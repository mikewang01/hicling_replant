...
static volatile bool uart_tx_done = false;  // UART write complete
...
static bool piggy_back_partition_table(uint8_t *ram)
{
        ...
        do {
                flash_copy_to_ram(flash_addr, (uint8_t *)&flash_entry, sizeof(partition_entry_t),
                                                                       &flash_ops_winbond_qspi);
...
static bool cmd_send_to_ram(HANDLER_OP hop)
{
        ...
        case HOP_EXEC:
                /*
                 * do not try to copy data into exactly the same place, this is especially important
                 * when 'magic' address for temporary buffer is used since it's the same as payload
                 * data buffer
                 */
                if (hdr->ptr != cmd_state.data) {
                        memmove(hdr->ptr, cmd_state.data, cmd_state.data_len);
                }
                xmit_ack();
                return true;
...
static bool cmd_write_ram_to_qspi(HANDLER_OP hop)
{
        ...
        case HOP_EXEC:
                if (flash_copy_from_ram(hdr->ptr, hdr->len, hdr->addr, &flash_ops_winbond_qspi)) {
                        xmit_ack();
                } else {
                        xmit_nak();
                }
...
static bool cmd_erase_qspi(HANDLER_OP hop)
{
        ...
        case HOP_EXEC:
                if (flash_erase_region(hdr->addr, hdr->len, &flash_ops_winbond_qspi)) {
                        xmit_ack();
                } else {
                        xmit_nak();
                }
...
static bool cmd_read_qspi(HANDLER_OP hop)
{
        ...
        case HOP_EXEC:
                /* there's no payload for this command so we can safely read into buffer */
                flash_copy_to_ram(hdr->addr, cmd_state.data, hdr->len, &flash_ops_winbond_qspi);
                return true;
...
static bool cmd_is_empty_qspi(HANDLER_OP hop)
{
        ...
        case HOP_EXEC:
                ...
                while (i < hdr->size) {
                        ...
                        flash_copy_to_ram(hdr->start_address + i, (uint8_t *) tmp_addr2,
                                                                read_len, &flash_ops_winbond_qspi);
...
static bool cmd_chip_erase_qspi(HANDLER_OP hop)
{
        ...
        case HOP_EXEC:
                ...
                if (flash_chip_erase(&flash_ops_winbond_qspi)) {
                        return true;
                } else {
                        return false;
                }
...
static void init(void)
{
        ...
        flash_init(&flash_ops_winbond_qspi);
}
