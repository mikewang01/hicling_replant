...
#define I2C_DEVICE_DEF(bus_id, bus_res_id, dynamic_data, name, _mode, _address, _addr_mode, \
                                                                        _speed, _dma_channel) \
        i2c_dev_dynamic_data dev_data_##name; \
...
#define I2C_BUS(bus_id) \
        i2c_bus_dynamic_data dynamic_##bus_id;
 ...
 */
static PRIVILEGED_DATA const i2c_device_config *current_config[2];
...
static void ad_i2c_event_cb(void *user_data, HW_I2C_ABORT_SOURCE error)
{
        i2c_cb_data *data = (i2c_cb_data *) user_data;
        const i2c_device_config *device = data->config;
        data->success = error == 0;
        data->abort_source = error;
        OS_EVENT_SIGNAL_FROM_ISR(device->bus_data->event);
}
...
int ad_i2c_transact(i2c_device dev, const uint8_t *wbuf, size_t wlen, uint8_t *rbuf, size_t rlen)
{
        ...
        ad_i2c_async_transact(dev, I2C_SND(wbuf, wlen),
                                I2C_RCV(rbuf, rlen),
                                I2C_CB1(ad_i2c_event_cb, &data),
                                I2C_END);
        OS_EVENT_WAIT(config->bus_data->event, OS_EVENT_FOREVER);

        return data.abort_source;
}

int ad_i2c_write(i2c_device dev, const uint8_t *wbuf, size_t wlen)
{
        i2c_device_config *config = (i2c_device_config *) dev;
        HW_I2C_ID id = config->bus_id;
        i2c_cb_data data;
        data.config = config;
        data.success = true;
        data.abort_source = 0;
        ...
        hw_i2c_write_buffer_async(id, wbuf, wlen, ad_i2c_transaction_cb, &data, HW_I2C_F_WAIT_FOR_STOP);
        OS_EVENT_WAIT(config->bus_data->event, OS_EVENT_FOREVER);
...
int ad_i2c_read(i2c_device dev, uint8_t *rbuf, size_t rlen)
{
        ...
        hw_i2c_read_buffer_async(id, rbuf, rlen, ad_i2c_transaction_cb, &data, HW_I2C_F_NONE);
        OS_EVENT_WAIT(config->bus_data->event, OS_EVENT_FOREVER);
...
void ad_i2c_async_transact(i2c_device dev, ...)
{
        ...
        /*
         * Reset abort/start/stop conditions.
         */
        hw_i2c_reset_abort_source(id);
        hw_i2c_reset_int_stop_detected(id);
        hw_i2c_reset_int_start_detected(id);
