...
#define SPI_SLAVE_DEVICE(bus, name, cs_port, cs_pin, _word_mode, pol_mode, _phase_mode, xtal_div, dma_channel) \
        spi_dev_dynamic_data dev_data_##name; \
...
#define SPI_BUS(bus_id) \
        spi_bus_dynamic_data dynamic_##bus_id;
 ...
 */
static PRIVILEGED_DATA const spi_device_config *current_config[2];
...
{
        spi_device_config *config = (spi_device_config *) dev_id;

#if CONFIG_SPI_EXCLUSIVE_OPEN
        resource_acquire(DEVICE_RESOURCE_MASK(config), OS_EVENT_FOREVER);
#endif

        return (spi_device) config;
}

void ad_spi_close(spi_device dev)
{
#if CONFIG_SPI_EXCLUSIVE_OPEN
        const spi_device_config *config = (spi_device_config *) dev;
