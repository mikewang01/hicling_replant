 ...
 *
 * Activate chip select after access to bus is granted. Wait if bus is used by other device
 * transmission.
 *
 * \param [in] dev handle to SPI device
 *
 ...
 *
 * Deactivate chip select and release access to SPI bus.
 *
 * \param [in] dev handle to SPI device
 *
 */
...
#define ad_spi_device_acquire(dev)
#define ad_spi_device_release(dev)
 ...
 */
typedef struct {
        OS_EVENT event;        /**< Event used for synchronization in accessing SPI controller */
        struct spi_device_config *current_device; /**< This keeps track of last device that was
                                 used. When device changes all controller parameters must also be
                                 changed. This field is cleared when platform enters sleep mode */
...
typedef struct {
        ...
        int8_t bus_acquire_count; /**< this keeps track of number of calls to ad_spi_bus_acquire() */
 ...
 * actions will be executed in interrupt context.
 ...
 * - Callback execution         SPI_CB1 SPI_CB0
 *
...
typedef struct spi_device_config {
        ...
        RES_ID bus_res_id;              /**< SPI resource ID RES_ID_SPI1 or RES_ID_SPI2 */
        spi_config hw_init;
        bool ignore_cs;                 /**< true if in slave mode CS line should be ignored */
        spi_bus_dynamic_data *bus_data; /**< pointer to dynamic bus data */
        spi_dev_dynamic_data *data;     /**< pointer to dynamic device data */
#if CONFIG_SPI_RESOURCE_STATIC_ID
        RES_ID dev_res_id;              /**< If CONFIG_SPI_RESOURCE_STATIC_ID is defined, RES_ID
                                        contains value for this device and it can be stored in
                                        constant data instead of dynamic */
#endif
} spi_device_config;
