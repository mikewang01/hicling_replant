...
#if (dg_configBLE_CONN_PARAM_REQ_DISABLE)
#include "gapc.h"
#include "gapc_task.h"
#endif
...
static int sleep_when_possible(void)
{
        ...
        do {
                ...
                if (CHECK_AND_CALL(custom_presleepcheck) != 0) {
                        if (ble_block()) {
                ...
                ret = ble_sleep(sleep_for_ever, &sleep_duration_in_lp);
                if (ret == 1) {
...
#ifdef BLE_PROD_TEST
static void read_public_address(void)
{
        static const uint8_t default_addr[BD_ADDR_LEN] = defaultBLE_STATIC_ADDRESS;
...
static void read_public_address(void)
{
        ...
        param_len = ad_nvparam_get_length(ble_parameters, TAG_BLE_PLATFORM_BD_ADDRESS, NULL);
        if (param_len < sizeof(public_address) + sizeof(valid)) {
                goto done;
        }

        ad_nvparam_read_offset(ble_parameters, TAG_BLE_PLATFORM_BD_ADDRESS,
                                        param_len - sizeof(valid), sizeof(valid), &valid);

        /* Read BD address from nvparam only if validity flag is set to 0x00 */
        if (valid == 0x00) {
                read_len = ad_nvparam_read(ble_parameters, TAG_BLE_PLATFORM_BD_ADDRESS,
                                                        sizeof(public_address), public_address);
        }

done:
        /* If address read from nvparam is invalid, replace with default one */
        if (read_len != sizeof(public_address)) {
                memcpy(public_address, &default_addr, sizeof(public_address));
        }
#else
        static const uint8_t empty_nvms_addr[BD_ADDR_LEN] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
        nvms_t nvms;

        nvms = ad_nvms_open(NVMS_PARAM_PART);

        ad_nvms_read(nvms, NVMS_PARAMS_TAG_BD_ADDRESS_OFFSET, (uint8_t *) public_address,
                                                                        sizeof(public_address));

        /* If address is read from empty flash, replace with default one */
        if (!memcmp(public_address, empty_nvms_addr, sizeof(public_address))) {
                memcpy(public_address, &default_addr, sizeof(public_address));
        }
#endif
}
...
static void ad_ble_task(void *pvParameters)
{
        ...
        for (;;) {
                ...
                while ((sleep_status == BLE_ACTIVE) && (ret != -1)) {
                        ...
                        else if ( (USE_BLE_SLEEP == 1) && (cm_lp_clk_is_avail()) ) {
...
#if ((dg_configBLE_ADV_STOP_DELAY_ENABLE == 1) || (dg_configBLE_CONN_PARAM_REQ_DISABLE == 1))
        ble_gtl_msg_t *gtl_msg = (ble_gtl_msg_t *) &(ptr_msg->msg.gtl);

#if (dg_configBLE_CONN_PARAM_REQ_DISABLE == 1)
        if ((msg_type == GTL_MSG)
                && (gtl_msg->msg_id == GAPC_PARAM_UPDATE_CMD)) {
                gapc_env[(gtl_msg->src_id)>>8]->features &=
...
const uint8_t *ad_ble_get_public_address(void)
{
