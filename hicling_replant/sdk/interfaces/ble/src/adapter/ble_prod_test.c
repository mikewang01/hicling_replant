...
static void send_ble_evt(uint16_t cmd_opcode, uint8_t event_code, uint8_t param_length, uint8_t *params_buf)
{
	...
	msgBuf = OS_MALLOC(sizeof( irb_ble_stack_msg_t ) + param_length - sizeof(uint8_t));

	msgBuf->op_code = IRB_BLE_STACK_MSG;                // fill IRB message OP code
	msgBuf->msg_type = HCI_EVT_MSG;                  // fill stack message type
	msgBuf->msg_size = HCI_EVT_HEADER_LENGTH + param_length;    // fill total message size
...
static void start_pkt_rx_stats(const irb_ble_stack_msg_t *stack_msg)
{
...
static void stop_pkt_rx_stats(void)
{
	...
	params[0] = prod_test_rx_packet_nr & 0xFF;
	params[1] = (prod_test_rx_packet_nr >> 8) & 0xFF;
	params[2] = prod_test_rx_packet_nr_syncerr & 0xFF;
	params[3] = (prod_test_rx_packet_nr_syncerr >> 8) & 0xFF;
	params[4] = prod_test_rx_packet_nr_crcerr & 0xFF;
	params[5] = (prod_test_rx_packet_nr_crcerr >> 8) & 0xFF;
	params[6] = prod_test_rx_test_rssi & 0xFF;
	params[7] = (prod_test_rx_test_rssi >> 8) & 0xFF;

	send_ble_evt(BPT_OPCODE_STOP_PKT_RX_STATS, BPT_EVTCODE_STOP_PKT_RX_STATS,
				BPT_PLEN_STOP_PKT_RX_STATS, params);
...
static void start_cont_tx(const irb_ble_stack_msg_t *stack_msg)
{
...
void stop_pkt_tx_interval(bool auto_stop)
{
        ...
        if (auto_stop)
                send_ble_evt(BPT_OPCODE_PKT_TX_INTV, BPT_EVTCODE_PKT_TX_INTV_COMPLETE,
                        BPT_PLEN_PKT_TX_INTV, NULL);
        else
                send_ble_evt(BPT_OPCODE_STOP_CONT_TX, BPT_EVTCODE_STOP_CONT_TX,
                        BPT_PLEN_STOP_CONT_TX, NULL);

        /* Make sure the BLE state here is either IDLE or STOPPING */
        configASSERT(st == LLM_IDLE || st == LLM_STOPPING);
}
...
/* Continuous modulated, non-packetized tx */
static void pkt_tx_interval(const irb_ble_stack_msg_t *stack_msg)
{
...
static void unmodulated_trx(const irb_ble_stack_msg_t *stack_msg)
{
        uint8_t oper;
        uint8_t freq;
        ...
        if (stack_msg->msg.hci.cmd.param_length == 2) {
                ...
                if (oper == 0x54) { /* Unmodulated TX */
                        if (prod_test_state == STATE_PROD_TEST_IDLE) {
                                prod_test_state = STATE_PROD_UNMODULATED_TX;
                                hw_rf_start_continuous_wave(0x1, freq);
                        }
                }
                else if (oper == 0x4f) { /* OFF */
                        if (prod_test_state == STATE_PROD_UNMODULATED_TX) {
...
void ble_prod_test_cmd(const irb_ble_stack_msg_t *stack_msg)
{
