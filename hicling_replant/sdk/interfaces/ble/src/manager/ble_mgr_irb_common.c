...
void ble_adapter_cmp_evt_init(ad_ble_msg_t *ad_msg, void *param)
{
         ...
         * set. Now it's good time to update dev params - we always start with public static address
         * and application can change this after BLE is enabled.
         */
        devp = ble_mgr_dev_params_acquire();
        devp->own_addr.addr_type = PUBLIC_STATIC_ADDRESS;
        memcpy(devp->own_addr.addr, ad_ble_get_public_address(), sizeof(devp->own_addr.addr));
        ble_mgr_dev_params_release();
...
void irb_ble_handler_enable_cmd(OS_IRB *irb)
{
        ...
        // Add expected response on the waitqueue -- IS NOT GTL!
