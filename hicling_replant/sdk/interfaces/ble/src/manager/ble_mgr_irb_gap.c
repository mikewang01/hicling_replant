...
static void gapc_disconnect_cmd_rsp(ble_gtl_msg_t *gtl, void *param)
{
        OS_IRB *irb = param;
        irb_ble_gap_disconnect_rsp_t *rsp = irb->ptr_buf;
...
void irb_ble_handler_gap_disconnect_cmd(OS_IRB *irb)
{
        ...
        ble_gtl_waitqueue_add(cmd->conn_idx, GAPC_CMP_EVT, GAPC_DISCONNECT, gapc_disconnect_cmd_rsp, irb);
...
static void send_bond_cmd(uint16_t conn_idx, gap_io_cap_t io_cap, bool bond, bool mitm)
{
        ...
        gcmd->pairing.sec_req = GAP_SEC1_NOAUTH_PAIR_ENC;
...
void irb_ble_handler_gapc_cmp__encrypt_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* send to event queue */
        ble_mgr_event_queue_send(&irb, portMAX_DELAY);
