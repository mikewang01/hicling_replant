...
{
        printf("%s: conn_idx=%04x handle=%04x\r\n", __func__, evt->conn_idx, evt->handle);
...
void ble_central_task(void *params)
{
        ...
        const gap_conn_params_t cp = {
                .interval_min = 0x28, // 50.00ms
                .interval_max = 0x38, // 70.00ms
                .slave_latency = 0,
                .sup_timeout = 0x2a,  // 420ms
        };
...
no_event:
                ...
                if (notif & DISCOVER_NOTIF) {
                        ...
                        ble_uuid_create16(0x1812, &uuid);
                        ble_gattc_browse(0, &uuid);
#else
