...
static void system_init( void *pvParameters )
{
        ...
        OS_TASK_CREATE("BLE Peripheral",                /* The text name assigned to the task, for
                       ...
                       &task_hanles,                            /* The parameter passed to the task. */
                       ...
                       task_hanles.ble_handle);                         /* The task handle. */
        OS_ASSERT(handle);

        OS_TASK_CREATE("cling fsm",                /* The text name assigned to the task, for
                                                           debug only; not used by the kernel. */
                       cling_task,             /* The function that implements the task. */
                       &task_hanles,                            /* The parameter passed to the task. */
                       200 * OS_STACK_WORD_SIZE,        /* The number of bytes to allocate to the
 stack of the task. */
                       maincling_FSM_TASK_PRIORITY,/* The priority assigned to the task. */
                       task_hanles.cling_handle);                         /* The task handle. */
        OS_ASSERT(handle);
