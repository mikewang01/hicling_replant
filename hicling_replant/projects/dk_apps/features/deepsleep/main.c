 ...
 *
 * Copyright (C) 2015 - 2016. Dialog Semiconductor Ltd, unpublished work. This computer
...
#define INITIAL_SLEEP_MODE      pm_mode_deep_sleep
#endif
...
static OS_TASK ButtonHandle = NULL;
...
{
        // Do nothing!
        NVIC_ClearPendingIRQ(SWTIM0_IRQn);
}
/*-----------------------------------------------------------*/
...
static void system_init(void *pvParameters)
{
        ...
        status = OS_TASK_CREATE("Button",                                       /* The text name assigned to the task - for debug only as it is not used by the kernel. */
                                prvButtonTask,                                  /* The function that implements the task. */
                                ( void * ) NULL,                                /* The parameter passed to the task - just to check the functionality. */
                                configMINIMAL_STACK_SIZE * OS_STACK_WORD_SIZE,  /* The size of the stack to allocate to the task (in bytes). */
                                ( OS_TASK_PRIORITY_NORMAL ),                    /* The priority assigned to the task. */
                                ButtonHandle);                                  /* The task handle. */
        OS_ASSERT(ButtonHandle);
