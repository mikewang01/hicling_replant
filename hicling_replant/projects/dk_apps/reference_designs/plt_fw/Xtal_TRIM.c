...
#if debugging
	long i = 0;
	unsigned int start_TRIM = 1100;
	char flag_led = 0;
#endif

	hw_gpio_set_pin_function(1, 0, HW_GPIO_MODE_OUTPUT, HW_GPIO_FUNC_UART_TX);
	//hw_gpio_set_pin_function(1, 5, HW_GPIO_MODE_OUTPUT, HW_GPIO_FUNC_UART_RX);

	while(1)
	{
		at = auto_trim(42);	// example is p42 as input for the 500 msec signal
