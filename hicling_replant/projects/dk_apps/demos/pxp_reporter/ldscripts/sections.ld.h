...
SECTIONS
{
        ...
        RETENTION_RAM0 (NOLOAD) :
        {
                ...
                *(privileged_data_zi)
                *(retention_mem_zi)
                /* if proj_configRETRAM_CONFIG == 1 then os_heap and ble_msg_heap should be placed
                 * inside RetRAM1. */
                *(os_heap)
                *(ble_msg_heap)
                *(ble_env_heap)
        ...
        RETENTION_RAM1 (NOLOAD) :
        {
                . = ALIGN(4);
                __RetRAM1_start__ = .;
                /* if proj_configRETRAM_CONFIG == 0 then os_heap and ble_msg_heap should be placed
                 * inside RetRAM0. */
                . = ALIGN(4);
                __RetRAM1_end__ = .;
        } > RetRAM1
	...
	ASSERT(0, "Unsupported chip version")
        ...
        INCLUDE da14681_00_rom.symbols
                ...
                && (dg_configBLACK_ORCA_IC_STEP == BLACK_ORCA_IC_STEP_E))
                        /* Check if RAM does not overlap ROM variables */
        ASSERT(ORIGIN(RAM) + LENGTH(RAM) < 0x07FDC000, "ROM variables region (starting at 0x07FDC000) overflowed by RAM")
