...
#define PARAM_NAME_STOP_PRV_GDB_SERVER  "stop_prv_gdb_server"

/* bin2image parameters */
#define PARAM_NAME_CHIP_REV             "chip_rev"
#define PARAM_NAME_ENABLE_UART          "enable_uart"
#define PARAM_NAME_RAM_SHUFFLING        "ram_shuffling"
...
bool cli_config_save_to_ini_file(const char *file_path, const struct cli_options *opts)
{
        ...
        add_num_value(&sections_queue, SECTION_NAME_GDB_SERVER, PARAM_NAME_STOP_PRV_GDB_SERVER,
                                                        opts->gdb_server_config.stop_prv_gdb_server);

        /* bin2image section */
        ini_queue_add(&sections_queue, SECTION_NAME_BIN2IMAGE, PARAM_NAME_CHIP_REV,
                                                                                opts->chip_rev);
        ini_queue_add(&sections_queue, SECTION_NAME_BIN2IMAGE, PARAM_NAME_ENABLE_UART,
                                                                opts->enable_uart ? "y" : "n");
        add_num_value(&sections_queue, SECTION_NAME_BIN2IMAGE, PARAM_NAME_RAM_SHUFFLING,
                                                                opts->ram_shuffling);
...
bool cli_config_load_from_ini_file(const char *file_path, struct cli_options *opts)
{
        ...
        while ((elem = ini_queue_pop(&sections_queue)).section != NULL) {
                ...
                if (!strcmp(elem.section, SECTION_NAME_CLI)) {
                       ...
                       } else if (!strcmp(elem.key, PARAM_NAME_STOP_PRV_GDB_SERVER)) {
                               if (get_number(elem.value, &tmp)) {
                                       opts->gdb_server_config.stop_prv_gdb_server = (bool) tmp;
                               }
                       }
                        ...
                        } else if (!strcmp(elem.key, PARAM_NAME_ENABLE_UART)) {
                                opts->enable_uart = tolower(*elem.value) == 'y';
                        } else if (!strcmp(elem.key, PARAM_NAME_RAM_SHUFFLING)) {
                                if (get_number(elem.value, &tmp)) {
                                        opts->ram_shuffling = tmp;
                                }
                        }
                }
