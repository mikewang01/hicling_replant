...
/* Convert image in buffer to cached version */
int change_to_cached_image(uint8_t *buf, int size)
{
        uint8_t header[16];
        chip_info_t chip_info;
        int err = 0;
        int cached_header_size;
        ...
        err = prog_fill_image_header(header, size, chip_info.chip_rev,
                        main_opts.enable_uart, main_opts.ram_shuffling, IMG_QSPI, IMG_CACHED);

        if (err < 0) {
                return err;
        }

        cached_header_size = err;

        /* Make space for header, some bytes will be lost forever */
        memmove(buf + cached_header_size, buf, 0x100 - cached_header_size);
        memcpy(buf, &header, cached_header_size);

        return cached_header_size;
}
...
static int prog_write_qspi_exec(uint8_t *buf, int size)
{
        ...
        if (stack_pointer > 0x7FE0000 || stack_pointer < 0x7FC0000 || reset_handler < 0x8000100) {
                return ERR_PROG_QSPI_IMAGE_FORMAT;
        }

        err = change_to_cached_image(buf, size);
        if (err >= 0)
                err = prog_write_image_to_qspi_safe(buf, size);
...
static int cmdh_read_qspi(int argc, char *argv[])
{
        ...
        if (ret) {
                free(buf);
                fprintf(stderr, "read from RAM failed (%d)\n", ret);
                return 0;
        }
