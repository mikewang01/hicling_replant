...
int prog_read_chip_info(chip_info_t *chip_info)
{
        ...
        //AD chip requires special handling as OTP is not available
        if (strncmp((char*)id, "680AD", 5) == 0) {
                strncpy((char*)chip_otp_id, "DA14681 ", OTP_HEADER_CHIP_ID_LEN);
        }
        else {
                err = target->cmd_read_otp(((OTP_HEADER_CHIP_ID & ~OTP_START_ADDRESS)>>3), chip_otp_id, OTP_HEADER_CHIP_ID_LEN);
                if (err) {
                        return err;
                }
        }
        ...
        /* Read package info*/
        err = target->cmd_read_otp(((OTP_HEADER_POS_PACK_INFO & ~OTP_START_ADDRESS)>>3), chip_package, OTP_HEADER_POS_PACK_INFO_LEN);
        if (err) {
                return err;
        }
