...
static void stop_gdb_server_instances(const char *new_path)
{
        char pkill_cmd[300];
        int ret;
#ifdef WIN32
...
static bool is_running_gdb_server(const char *new_path)
{
#ifdef WIN32
        HANDLE snap_shot;
        ...
        strcpy(pkgrep_cmd, "pgrep ");
        get_name_from_path(new_path, &pkgrep_cmd[strlen(pkgrep_cmd)]);

        cmd = popen(pkgrep_cmd, "r");
        ...
        if ((fgets(cmd_out, sizeof(cmd_out), cmd) != 0) && (strlen(cmd_out) > 0)) {
                pclose(cmd);
                return true;
        }
...
static void run_command(const char *cmd)
{
        ...
        if (pid == 0) {
                ...
                /* Close standard input and standard error streams */
                close(STDIN_FILENO);
                close(STDERR_FILENO);
                /* Save to cli_gdb_server.log GDB Server output */
                f = freopen("cli_gdb_server.log", "w+", stdout);
                ret = system(cmd);
                fclose(f);
                (void) ret;
        } else if (pid == -1) {
...
static int gdb_server_init(const prog_gdb_server_config_t *gdb_server_conf)
{
        ...
        if (gdb_server_conf->stop_prv_gdb_server && gdb_server_conf->gdb_server_path &&
                ...
                stop_gdb_server_instances(gdb_server_conf->gdb_server_path);

                start_gdb = true;
        } else if (is_localhost(gdb_server_conf->host_name) && gdb_server_conf->gdb_server_path &&
                                        !is_running_gdb_server(gdb_server_conf->gdb_server_path)) {
                start_gdb = true;
        }
        ...
        /* set speed to 2000 kHz */
        if ((status = gdb_server_send_local_cmd("speed 2000")) != 0) {
                return status;
        }
        ...
        tmp_32 = 0x00000001;
        /* verification of this writting could failed - not readable register */
...
int gdb_server_cmd_write_otp(uint32_t address, const uint32_t *buf, uint32_t len)
{
        ...
        header_buf[2] = (uint8_t) total_len;
        header_buf[3] = (uint8_t) (total_len >> 8);
        ...
        /* send data */
        gdb_server_send_write_cmd(swd_addr.buf_addr, size, (const uint8_t *) buf);
        ...
        /* After write was executed, check if command was ACK'ed or NAK'ed */
...
void gdb_server_close(void)
{
        ...
        /* GDB Server process is running - send SIGKILL signal to it */
        kill(gdb_server_pid, SIGKILL);
        /* GDB Server process should receive SIGKILL signal and stops - wait for it */
        tmp_pid = wait(&status);
