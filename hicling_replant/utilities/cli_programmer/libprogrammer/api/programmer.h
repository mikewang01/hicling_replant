...
typedef struct {
        ...
        /* True if previous instances of GDB Server should be stopped */
 ...
 *
 */
void DLLEXPORT prog_close_interface(void);

/**
 * \brief Close serial port
 *
 */
void DLLEXPORT prog_serial_close(void);

/**
 * \brief Write to DA14680 to RAM memory
 *
...
/**
 * \brief Write file to DA14680 to RAM memory
 *
 * \param [in] ram_address RAM address, where file will be written
...
/**
 * \brief Write buffer to DA14680 to QSPI flash memory
 *
...
/**
 * \brief Write file to DA14680 to QSPI flash memory
 *
 * \param [in] flash_address QSPI flash address, where file will be written
 * \param [in] file_name Name of the file to written
 * \param [in] size Number of bytes to write
 ...
 *
 * Product_id must contain null terminated strings 'DA14681-00', 'DA14681-01', or
 * any future version of product id.
 * Chip_rev contains null terminated strings '680AA', '680AB', '680AC', '680AD' or
 * any future version of chip revision
 ...
 *              8 byte for chip revision 680AA, 680AB, 680AC, and 16 bytes for 680AD
 * \param [in] image_size size of bin file
 * \param [in] chip_rev string representing chip revision 680AC or 680AD
 * \param [in] enable_uart if true image will allow to boot from UART in AD chip
 * \param [in] ram_shuffling value 0-3 reflecting memory shuffling for AD chip
 ...
 * \param [in] chip_rev string representing chip revision 680AC or 680AD
 * \param [in] enable_uart if true image will allow to boot from UART in 680AD chip QSPI modes
 * \param [in] ram_shuffling value 0-3 reflecting memory shuffling for 680AD chip
 * \param [in] type specifies one of QSPI or OTP images
 * \param [in] mode specifies mirrored or cached mode
 * \param [out] buf pointer too buffer for prepared image (can be NULL)
 * \param [in] buf_size size of memory pointer by buf (must be 0 if buf is NULL)
 * \param [out] required_size pointer to variable that will receive required image size if
 *              buffer provided by caller is to small to fit image (can be NULL)
 *
...
int DLLEXPORT prog_make_image(uint8_t *binary, int binary_size, const char *chip_rev,
                                bool enable_uart, uint8_t ram_shuffling, image_type_t type,
                                image_mode_t mode, uint8_t *buf, int buf_size, int *required_size);
