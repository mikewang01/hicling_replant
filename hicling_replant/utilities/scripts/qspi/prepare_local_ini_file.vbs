...
if IsEmpty(id) then
  ...
  Do While result.stdOut.AtEndOfStream <> True
    ...
    'if line contains Serial number patter, get it
...
Do While InFile.AtEndOfStream <> True
    ...
    If 0 < InStr(TextLine, "gdb_server_path") Then
      ' Reular expresion for deleting old device id from ini file
      Set re = new regexp
      re.Global = true
      ...
      If Len(TextLine) < 20 Then
        TextLine = "gdb_server_path = " + gdb_path + " -if SWD -device Cortex-M0 -singlerun -silent -speed auto"
      End If
